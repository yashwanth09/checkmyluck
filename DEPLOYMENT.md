...
2. **New Project** → name it e.g. `checkmyluck` → Create.
...
1. Create a new repo on **[github.com](https://github.com)** (e.g. `checkmyluck`). Don’t add README if your folder already has one.
2. On your PC, open a terminal in the **checkmyluck** folder (where `package.json` is).
3. Run:
   ...
   git branch -M main
   git remote add origin https://github.com/YOUR_USERNAME/checkmyluck.git
...
2. **Add New** → **Project** → **Import** your `checkmyluck` repo.
...
4. Add these (use **Production**):
   ...
   | `UPI_PAYEE_NAME` | e.g. `CheckMyLuck` |
   | `CRON_SECRET` | Another long random string — for the 7 PM cron |

5. Click **Deploy**. Wait for the build to finish.
6. Your app is live at `https://checkmyluck-xxx.vercel.app` (or similar). Open it and check the home page.

---

### 4. Create tables and seed data (first time only)

1. On your PC, in the **checkmyluck** folder, set the production DB URL and run Prisma:
...
1. Buy **checkmyluck.com** from Namecheap, Cloudflare, Porkbun, or Squarespace if you don’t have it.
2. In **Vercel** → your project → **Settings** → **Domains** → **Add** → type `checkmyluck.com` → Add.
...
4. In your **domain registrar** dashboard, open **DNS** / **Manage DNS** for checkmyluck.com and add those records.
...
2. **Create cron job**:
   - **URL:** `https://checkmyluck.com/api/cron/close-groups` (or your Vercel URL if domain isn’t ready yet).
...
- [ ] Open **https://checkmyluck.com** — home page and live groups load.
- [ ] Go to **https://checkmyluck.com/admin** — log in with `ADMIN_SECRET`.
...
| **Domain** | Any registrar | ~₹500–800/year | Point CheckMyLuck.com to Vercel. |
...
2. Create a project (e.g. `checkmyluck`).
...
1. Push your code to **GitHub** (create a repo, push the `checkmyluck` folder).
...
4. Before deploying, set **Environment Variables** in the Vercel project:
   ...
   | `UPI_VPA` | Your UPI ID | e.g. `9876543210@paytm` |
   | `UPI_PAYEE_NAME` | CheckMyLuck | Name shown in UPI app |
...
2. In **Vercel** → Project → **Settings** → **Domains** → Add `checkmyluck.com` (and `www.checkmyluck.com` if you want).
...
2. **URL:** `https://checkmyluck.com/api/cron/close-groups`
